...
public class SimulationSetupPanel extends GElement{
	...
	private boolean pressedLast, setup, setVelocity, finished;
	...
	public void update(PApplet p) {
		...
		if(!setVelocity){
			...
			p.text(Integer.toString((int) (PApplet.dist(getActive().getX(), getActive().getY(), p.mouseX, p.mouseY)/10)) + "m/s", (getActive().getX() + p.mouseX)/2, (getActive().getY() + p.mouseY)/2);
			if(pressedLast && !p.mousePressed){
		...
		for(SetupSpaceObject object: objects){
			if (object.getActive() && !setVelocity){
				EventRegistry.fire(new Event(EventType.GUI_SLIDER_VALUE_SET, new String[]{"MassSlider", Float.toString(100*object.getMass()/2e4f)}));
				EventRegistry.fire(new Event(EventType.GUI_SLIDER_VALUE_SET, new String[]{"RadiusSlider", Float.toString(100*object.getRadius()/60f)}));
				panel.update(p);
				p.noFill();
				p.stroke(color3);
				p.rect(object.getX() - object.getRadius(), object.getY() - object.getRadius(), 2* object.getRadius(), 2* object.getRadius());
			}
			p.noStroke();
	...
	private void setupListeners(){
		...
		Main.getGUI().getState().addListener(new Listener("MassSlider", new Runnable(){
			...
			public void run() {
				...
				EventRegistry.fire(new Event(EventType.GUI_SLIDER_VALUE_SET, new String[]{"MassSlider", Float.toString(PApplet.map(getActive().getMass(), 0, 2e4f, 0, 100))}));			
			}
		...
		Main.getGUI().getState().addListener(new Listener("RadiusSlider", new Runnable(){
			...
			public void run() {
				...
				EventRegistry.fire(new Event(EventType.GUI_SLIDER_VALUE_SET, new String[]{"RadiusSlider", Float.toString(PApplet.map(getActive().getRadius(), 0, 60f, 0, 100))}));
			}
		...
		//TODO: isn't working
		Main.getGUI().getState().addListener(new Listener("SimulationPanel - reset", new Runnable(){
