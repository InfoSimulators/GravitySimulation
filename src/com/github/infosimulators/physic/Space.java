...
public class Space {
	...
	/**
	 * Stores the maximum distance from the center @see pointOfOrigin. Objects
	 * further appart are seen lost and will be removed from space register and
	 * deleted by GC.
	 */
	...
	public void addGravitationForces() {
		for (PhysicsObject object : spaceRegister) {
			for (PhysicsObject other : spaceRegister) {
				if (object == other)
					continue;
				// System.out.println("object: "+ object.mass + "other: " +
				// other.mass + " => " + gravitation(object, other));
				...
				if (Vector3.distance(object.position, other.position) < (object.size + other.size) / 1.5f) {
					...
					if (Vector3.sqrDistance(object.velocity, other.velocity) < gravitationForce.sqrMagnitude()) {
						...
						spaceRegister.add(new PhysicsObject(
								Vector3.lerp(object.position, other.position, other.mass / (object.mass + other.mass)),
								Vector3.add(object.velocity, other.velocity)
	...
	/**
	 * Calculats the force PhysicsObject b acts on PhysicsObject a. Only the
	...
	public static Vector3 gravitation(PhysicsObject a, PhysicsObject b) {
		float force = (float) Constants.G
				* ((a.mass * b.mass) / (Vector3.sqrDistance(a.position, b.position) * 1000 * 1000));
