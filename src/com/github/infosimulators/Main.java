...
public class Main {
	...
	public static Simulation workingExample() {
		Simulation x = new Simulation(new float[][] { { 0f, (float) Math.PI, 10, 0f, 0f, 1f, 3f },
				{ 10f, (float) Math.PI, 10, 0f, 0f, 1f, 3f } });
		// Simulation.test(x, 5);
		return x;
	}
		...
		classicModeState.addListener(new Listener("FinishButton", new Runnable() {
			...
			public void run() {
				...
				Main.getGUI().getState()
						.addElement(new Text("ChangeDotsText", "Center shown:", 30, PApplet.LEFT, 150, 20, 100, 50));
				Main.getGUI().getState()
						.addElement(new CheckBox("ChangeDotsCheckBox", false, 380, 10, 40, 40));
